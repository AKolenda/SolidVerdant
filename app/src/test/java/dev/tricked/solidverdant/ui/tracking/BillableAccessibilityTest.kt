 ...
 * [Role.Switch] plus a checked/unchecked toggle state AND the "Billable" text label merged
 * together — previously it was a bare, unlabeled `Checkbox` that a screen reader announced with
 * no name. [EditTimeEntryDialog] hosts the row via a grouped switch row (`Modifier.toggleable(..., role = Role.Switch)`)
...
class BillableAccessibilityTest {
    ...
    /** [Role.Switch] merged with the checked [ToggleableState] and the "Billable" text label, as one node. */
    private fun billableCheckboxMatcher(checked: Boolean): SemanticsMatcher {
        ...
        return SemanticsMatcher("Role.Switch + ToggleableState($expectedState) + text 'Billable'") { node ->
            val hasRole = node.config.getOrElseNullable(SemanticsProperties.Role) { null } == Role.Switch
    ...
    fun billable_row_is_a_single_operable_toggle_node() {
        ...
        // The merged Role.Switch row (carrying the "Billable" label, not just the small Switch
